...
/**
 * Complete the information of the device with the information in the configuration group 
 * (with precedence of the device). The first argument indicates what fields would be merged.
 *
...
function mergeDeviceWithConfiguration(fields, defaults, deviceData, configuration, callback) {
    ...
    if (configuration){
        deviceData.cbHost = deviceData.cbHost || configuration.cbHost;
        deviceData.ngsiVersion = deviceData.ngsiVersion || configuration.ngsiVersion;
    }
...
function findConfigurationGroup(deviceObj, callback) {
    ...
    if (config.getConfig().singleConfigurationMode === true) {
        ...
        config
            .getGroupRegistry()
            .findTypeSilently(deviceObj.service, deviceObj.subservice, deviceObj.type, 
                deviceObj.apikey, handlerGroupFind);
    }
}
